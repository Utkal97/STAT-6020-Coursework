...
The graphs demonstrated above for the interest rate with bin-size of 5 looks like a Right skewed unimodal distribution.
...
ggplot(loans, aes(x = interest_rate, fill = grade)) +
  ...
  labs(
    x = "Interest Rate (%)",
    y = "Frequency",
    title = "Interest Rates grade wise in Lending Club loans"
  ) +
   facet_wrap(~ grade, nrow=3)
```
...
ggplot(loans %>% filter(grade != "G"), aes(x=loan_amount, y=interest_rate, fill=grade)) +
  geom_hex() +
  labs(title = "Hexplot of Interest Rates vs Loan Amount",
       x="Loan Amount",
       y="Interest Rate")
```
